/// GET - render email-confirmation-request-sent
/// POST - create user from request body and send email
...
async fn forgot_password_request(
    ...
    if let Err(e) = payload {
        return fastn_core::http::user_err(
            vec![
                ...
                ("username".into(), vec!["username/email is required".to_string()])
            ],
            fastn_core::http::StatusCode::OK,
        );
    }
    ...
    if payload.email_or_username.is_empty() {
        return fastn_core::http::user_err(
            vec![("username".into(), vec!["username/email is required".to_string()])],
            fastn_core::http::StatusCode::OK,
        );
    }
    ...
    let query = fastn_core::schema::fastn_user::table
        ...
        .select((fastn_core::auth::FastnUser::as_select(), fastn_core::schema::fastn_user_email::email));

    let user : Option<(fastn_core::auth::FastnUser, fastn_core::utils::CiString)>  = query
        .first(&mut conn)
        .await
        .optional()?;
    ...
    if user.is_none() {
        return fastn_core::http::user_err(
            vec![("username".into(), vec!["invalid email/username".to_string()])],
            fastn_core::http::StatusCode::OK,
        );
    }
...
fn reset_password_request_ftd() -> &'static str {
    r#"
    -- import: fastn/processors as pr

    -- auth.reset-password-request-sent:
    "#
}

/// create and return a mailer 
/// converting the `fastn_ds::EnvironmentError` to fastn_core::Error
async fn get_mailer(ds: &fastn_ds::DocumentStore) -> fastn_core::Result<fastn_core::mail::Mailer> {
    fastn_core::mail::Mailer::from_env(&ds).await
    .map_err(|e| {
        tracing::error!("failed to create mailer from env: {e}");
        fastn_core::Error::generic(
