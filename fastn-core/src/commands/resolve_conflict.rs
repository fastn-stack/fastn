pub async fn resolve_conflict(
    ...
    if number_of_times_flag_used > 1 {
        return fastn_core::usage_error("AmbiguousOptionError: Use only one flag".to_string());
    }
...
async fn get_conflict_data(
    config: &fastn_core::Config,
    file_status: &fastn_core::sync_utils::FileStatus,
) -> fastn_core::Result<ConflictData> {
    match file_status {
        ...
        fastn_core::sync_utils::FileStatus::Update {
            ...
            if !matches!(status, fastn_core::sync_utils::Status::Conflict(_)) {
                return fastn_core::usage_error(format!(
        ...
        fastn_core::sync_utils::FileStatus::Delete { path, status, .. } => {
            let version =
                if let fastn_core::sync_utils::Status::CloneDeletedRemoteEdited(version) = status {
                    version
                } else {
                    return fastn_core::usage_error(format!(
        ...
        fastn_core::sync_utils::FileStatus::Uptodate { path, version, .. } => {
