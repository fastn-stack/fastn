...
use fastn_cli_test_utils::{CommandOutput, detect_target_dir};
...
impl FastnRigTestEnv {
    ...
    pub async fn create_peer(&mut self, name: &str) -> Result<&PeerHandle, Box<dyn std::error::Error>> {
        ...
        let output = tokio::process::Command::new(binary_path)
            .arg("init")
            .env("SKIP_KEYRING", if self.skip_keyring { "true" } else { "false" })
    ...
    pub async fn start_peer(&mut self, peer_name: &str) -> Result<(), Box<dyn std::error::Error>> {
        let peer_index = self.peers.iter().position(|p| p.name == peer_name)
    ...
    pub async fn send_email(&self, from_peer: &str, to_peer: &str, subject: &str, body: &str) -> Result<CommandOutput, Box<dyn std::error::Error>> {
        let from = self.peers.iter().find(|p| p.name == from_peer)
            .ok_or(format!("From peer {from_peer} not found"))?;
        let to = self.peers.iter().find(|p| p.name == to_peer)  
        ...
        let output = tokio::process::Command::new(binary_path)
            .args([
                "send-mail",
                "--smtp", &from.smtp_port.to_string(),
                "--password", &from.password,
                "--from", &format!("test@{}.com", from.account_id),
                "--to", &format!("inbox@{}.com", to.account_id),
                "--subject", subject,
                "--body", body,
            ])
