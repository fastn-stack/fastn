...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_email_content = format!(
        ...
        from_email, to_email, args.subject, 
        chrono::Utc::now().timestamp(), args.body
    );
    ...
    let email_id = mail_store.smtp_receive(
        &from_email,
        &[to_email.clone()],
    ...
    match attempt_direct_delivery(sender_secret_key, &target_key, p2p_message).await {
        Ok(_) => {
            ...
            println!("✅ IMMEDIATE P2P delivery SUCCESS in {:.3}s", elapsed.as_secs_f64());
        ...
        Err(e) => {
            let elapsed = start_time.elapsed();
            println!("❌ IMMEDIATE P2P delivery FAILED in {:.3}s: {}", elapsed.as_secs_f64(), e);
        }
    }
