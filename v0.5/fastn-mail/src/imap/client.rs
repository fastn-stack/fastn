...
impl ImapClient {
    ...
    async fn connect_with_test_operations(&self, test_operations: bool) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "net")]
        {
            println!("🔗 Connecting to IMAP server {}:{}", self.config.host, self.config.port);
            println!("👤 Username: {}", self.config.username);
            println!("🔐 STARTTLS: {}", if self.config.starttls { "enabled" } else { "disabled" });

            // Connect to IMAP server
            let tcp_stream = tokio::net::TcpStream::connect((&*self.config.host, self.config.port)).await?;
            ...
            let mut imap_session = if self.config.starttls {
                ...
                client.login(&self.config.username, &self.config.password).await.map_err(|(err, _)| err)?
            } else {
                println!("📧 Using plain text connection");
    ...
    #[cfg(feature = "net")]
    async fn run_test_operations(&self, imap_session: &mut async_imap::Session<tokio_util::compat::Compat<tokio::net::TcpStream>>) -> Result<(), Box<dyn std::error::Error>> {
