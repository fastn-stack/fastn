...
impl Store {
    ...
    pub async fn imap_search(
        ...
        let sql = match criteria {
            ...
            "SEEN" => {
                "SELECT rowid FROM fastn_emails WHERE folder = ? AND is_deleted = 0 AND is_seen = 1"
            }
            "FLAGGED" => {
                "SELECT rowid FROM fastn_emails WHERE folder = ? AND is_deleted = 0 AND is_flagged = 1"
            }
            _ => {
                return Err(ImapSearchError::InvalidSearchCriteria {
    ...
    pub async fn imap_store_flags(
        ...
        let custom_flags: Vec<String> = flags
            .iter()
    ...
    pub async fn imap_expunge(&self, folder: &str) -> Result<Vec<u32>, ImapExpungeError> {
        ...
        for row in rows {
            let (uid, file_path) =
                row.map_err(|e| ImapExpungeError::DatabaseOperationFailed { source: e })?;
    ...
    pub async fn imap_thread(
        ...
        _folder: &str,
        algorithm: &str,
    ) -> Result<ThreadTree, ImapThreadError> {
