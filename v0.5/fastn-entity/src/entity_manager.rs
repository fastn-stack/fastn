...
impl EntityManager {
    ...
    pub async fn load_entity(&self, id52: &str) -> eyre::Result<fastn_entity::Entity> {
        ...
        fastn_entity::Entity::load(&entity_path)
            .await
            .wrap_err_with(|| format!("Failed to load entity '{id52}' from {entity_path:?}"))
    }
    ...
    pub async fn create_entity(&mut self) -> eyre::Result<fastn_entity::Entity> {
        ...
        let entity = fastn_entity::Entity::create(&self.path)
            .await
    ...
    fn save_config(&self) -> eyre::Result<()> {
        ...
        std::fs::write(&config_path, config_json).wrap_err("Failed to write config.json")?;
