...
impl Symbols {
    fn find_all_definitions_in_a_module(
        ...
        let d = fastn_unresolved::parse(module.clone(), &source, desugared_auto_imports, arena);
...
impl fastn_compiler::SymbolStore for Symbols {
    async fn lookup(
        ...
        unique_modules
            .into_iter()
            .flat_map(|m| self.find_all_definitions_in_a_module(arena, m, desugared_auto_imports))
            .collect()
    }
}
