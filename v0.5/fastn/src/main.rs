...
async fn main() {
    ...
    let mut config = fastn_core::Config::read(Default::default(), &mut interner)
        .await
        .unwrap();
    // read config here and pass to everyone?
    // do common build stuff here
    match command {
        fastn::commands::Cli::Serve(input) => input.run(config, interner).await,
        fastn::commands::Cli::Render(input) => input.run(&mut config, interner).await,
        fastn::commands::Cli::Build(input) => input.run(config, interner).await,
