use fastn_ascii_renderer::{
    TaffyLayoutEngine, FtdToCssMapper, SimpleFtdComponent, FtdSize,
    AnsiCanvas, CharPos, AnsiColor, CoordinateConverter, BorderStyle
};
...
fn test_column_layout_pipeline() {
    ...
    let child1_node = layout_engine.create_text_node("First", child1_style).unwrap();
    let child2_node = layout_engine.create_text_node("Second", child2_style).unwrap();

    // Create column container
    let column_style = css_mapper.component_to_style(&column);
    let column_node = layout_engine.create_container_node(column_style, vec![child1_node, child2_node]).unwrap();
    ...
    println!("Child 1: x:{}, y:{}, w:{}, h:{}", 
             child1_layout.location.x, child1_layout.location.y,
             child1_layout.size.width, child1_layout.size.height);
    println!("Child 2: x:{}, y:{}, w:{}, h:{}", 
             child2_layout.location.x, child2_layout.location.y,
             child2_layout.size.width, child2_layout.size.height);
}
