    ...
    ERROR: for<'de> serde::Deserialize<'de>,
{
    ...
    // We need to call fastn_net::get_stream but it needs graceful parameter
    // Since we can't use the singleton directly, we need to pass it through
    let (mut send_stream, mut recv_stream) = fastn_net::get_stream(
        endpoint,
        net_protocol.into(),
        target,
        crate::pool(),
        crate::graceful(), // This is the only internal exception for legacy fastn_net compatibility
    )
    .await
