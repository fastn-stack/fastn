...
#[track_caller]
fn check_invariants<'a, T: fastn_section::JDebug>(
    ...
    assert!(
        ...
        "Invariant violation: Parser advanced scanner but returned null without adding errors! Input: {:?}",
        source
    );
    ...
    assert!(
        ...
        "Invariant violation: Parser returned None without errors but didn't reset scanner! Input: {:?}",
        source
    );
    ...
    if has_result && errors_added {
        ...
        assert!(
            ...
            "Invariant violation: Parser consumed less than error spans! Scanner end: {}, Max error end: {}, Input: {:?}",
            end_pos,
            max_error_end,
            source
        );
...
>(
    ...
    // Check invariants
    check_invariants::<T>(&scanner, start_index, initial_error_count, &debug, source.as_str());
