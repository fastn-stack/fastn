...
pub enum ContinuationResult<C: Continuation + ?Sized> {
    Done(C::Output),
...
pub trait Continuation {
    type Output;
    type NeededInput;
    type NeededOutput: Default;
    // type Init;

    // fn init(init: Self::Init) -> ContinuationResult<Self>;
    fn continue_after(self, n: Self::NeededOutput) -> ContinuationResult<Self>;
}

pub fn consume<C: Continuation, F>(mut c: ContinuationResult<C>, f: F) -> C::Output
where
    F: Fn(C::NeededInput) -> C::NeededOutput,
{
    loop {
        match c {
            fastn_section::ContinuationResult::Stuck(ic, input) => {
                c = ic.continue_after(f(input));
            }
            fastn_section::ContinuationResult::Done(c) => {
                return c;
            }
        }
    }
