...
async fn test_full_mesh() {
        ...
        let task = tokio::spawn(async move {
            ...
            for msg_num in 1..=messages_per_sender {
                for (recv_idx, receiver_id52) in receiver_ids_clone.iter().enumerate() {
                    println!("📤 Sender #{} → Message #{} → Receiver #{}", 
                            sender_id, msg_num, recv_idx + 1);
                    ...
                    if sender_output.status.success() {
                        ...
                        if stdout.contains("\"status\":\"success\"") {
                            ...
                            println!("⚠️ Sender #{} → Receiver #{} no JSON", sender_id, recv_idx + 1);
                        }
                    } else {
                        println!("❌ Sender #{} → Receiver #{} FAILED", sender_id, recv_idx + 1);
                    }
            ...
            println!("🎯 Sender #{} completed: {}/{} messages successful", 
                    sender_id, sender_success_count, messages_per_sender * receiver_ids_clone.len());
    ...
    for (i, task) in sender_tasks.into_iter().enumerate() {
        match task.await {
            Ok(success_count) => {
                ...
                println!("✅ Sender #{} finished with {} successes", i + 1, success_count);
            }
            Err(e) => {
    ...
    println!("🎉 Full mesh test completed: {}/{} total messages successful", 
             total_success, expected_total);
    
    // Assert reasonable success rate for robust mesh testing
    assert!(total_success >= expected_total / 2, 
