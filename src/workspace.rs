...
/*pub(crate) struct CRWorkspace {
    pub cr: usize,
    pub workspace: std::collections::BTreeMap<String, fastn::workspace::WorkspaceEntry>,
}
...
impl CRWorkspace {
    ...
    pub(crate) fn insert_workspace_entry(&mut self, workspace: WorkspaceEntry) {
        let cr_file_path = format!("{}/{}", fastn::cr::cr_path(self.cr), workspace.filename);
        let workspace = fastn::workspace::WorkspaceEntry {
...
impl fastn::Config {
    pub(crate) async fn evaluate_clone_workspace(&self) -> fastn::Result<Vec<WorkspaceEntry>> {
    ...
    pub(crate) async fn create_clone_workspace(&self) -> fastn::Result<()> {
        let workspace_list = self.evaluate_clone_workspace().await?;
        self.write_workspace(workspace_list.as_slice()).await?;
        Ok(())
    }

    pub(crate) async fn write_clone_available_cr(&self, reserved_crs: &[i32]) -> fastn::Result<()> {
        use itertools::Itertools;
    ...
    pub async fn get_available_crs(&self) -> fastn::Result<Vec<i32>> {
    ...
    pub async fn extract_cr_number(&self) -> fastn::Result<i32> {
        let mut available_crs = self.get_available_crs().await?;
        if available_crs.is_empty() {
            return fastn::usage_error("No available cr number, try `fastn sync`".to_string());
        }
    ...
    pub(crate) async fn read_workspace(&self) -> fastn::Result<Vec<WorkspaceEntry>> {
        let workspace = {
            let workspace = tokio::fs::read_to_string(self.workspace_file());
            let lib = fastn::FastnLibrary::default();
            fastn::doc::parse_ftd("fastn", workspace.await?.as_str(), &lib)?
        };
    ...
    pub(crate) async fn get_workspace_map(
        &self,
    ) -> fastn::Result<std::collections::BTreeMap<String, fastn::workspace::WorkspaceEntry>> {
        Ok(self
            .read_workspace()
            .await?
            .into_iter()
    ...
    /*pub(crate) async fn get_cr_workspace(&self, cr_number: usize) -> fastn::Result<CRWorkspace> {
        let workspace = self
            .get_workspace_map()
            .await?
            .into_iter()
            ...
            .collect::<std::collections::BTreeMap<String, fastn::workspace::WorkspaceEntry>>();
        Ok(CRWorkspace::new(cr_number, workspace))
    }*/

    pub(crate) async fn write_workspace(&self, workspace: &[WorkspaceEntry]) -> fastn::Result<()> {
