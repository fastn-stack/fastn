...
/// If Interpreter have any operation other than interpreting like importing other document, etc...
/// and interpreter don't know what to do with next line, It will return with the state
/// StuckOnImport{ module: String, state: InternalState }
/// StuckOnProcessor { state: InternalState, state: ftd:p1:Section }
...
impl Interpreter {
    ...
    pub fn from(name: &str, source: &str) -> ftd::p1::Result<Self> {
        Ok(Interpreter::Parsed(InterpreterState {
            p1: ftd::p1::parse(source, name)?,
            ..Default::default()
        }))
    }
    ...
    pub fn start(
        ...
        let p1 = ftd::p1::parse(source, name)?;
        ...
        while iteration_index < p1.len() && p1[iteration_index].name == "import" {
            ...
            return Ok(Interpreter::StuckOnImport {module: });
    ...
    pub(crate) fn interpreter2(&self, name: &str, source: &str) -> ftd::p1::Result<InterpreterState> {
        let p1 = ftd::p1::parse(source, name)?;
        let mut aliases = default_aliases();

        for section in p1.iter().filter(|s| s.name.eq("import")) {
            ...
            aliases.insert(alias, library_name);
