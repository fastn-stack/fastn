pub async fn rm(config: &fastn::Config, file: &str, cr: Option<&str>) -> fastn::Result<()> {
    if let Some(cr) = cr {
...
async fn simple_rm(config: &fastn::Config, file: &str) -> fastn::Result<()> {
...
async fn cr_rm(config: &fastn::Config, file: &str, cr: usize) -> fastn::Result<()> {
    use itertools::Itertools;
    ...
    // create delete entry
    let mut deleted_files = fastn::cr::get_deleted_files(config, cr).await?;
    if deleted_files
        .iter()
        .map(|v| &v.filename)
        .contains(&file.to_string())
    {
        return fastn::usage_error(format!("{} is already deleted in CR#{}", file, cr));
    }
    deleted_files.push(fastn::cr::CRDeleted::new(file, file_edit.version));
    fastn::cr::create_deleted_files(config, cr, deleted_files.as_slice()).await?;
