...
async fn serve_file(config: &mut fastn::Config, path: &camino::Utf8Path) -> fastn::http::Response {
    let f = match config.get_file_and_package_by_id(path.as_str()).await {
        ...
        Err(e) => {
            ...
            return fastn::not_found!("fastn-Error: path: {}, {:?}", path, e);
        }
    };
    // Auth Stuff
    if !f.is_static() {
        let req = if let Some(ref r) = config.request {
            ...
            return fastn::server_error!("request not set");
        };
        ...
        match config.can_read(req, path.as_str(), true).await {
            Ok(can_read) => {
                if !can_read {
                    ...
                    return fastn::unauthorised!("You are unauthorized to access: {}", path);
                }
            }
            Err(e) => {
                tracing::error!(msg = "can_read-error", path = path.as_str());
                return fastn::server_error!("fastn-Error: can_read error: {}, {:?}", path, e);
            }
        };

        match fastn::package::app::can_read(config, path.as_str()).await {
            Ok(can_read) => {
                if !can_read {
                    tracing::error!(
            ...
            Err(err) => {
                ...
                return fastn::server_error!("fastn-Error: can_read error: {}, {:?}", path, err);
            }
        };
    }

    match f {
        fastn::File::Ftd(main_document) => {
            match fastn::package::package_doc::read_ftd(config, &main_document, "/", false).await {
                Ok(r) => fastn::http::ok_with_content_type(r, mime_guess::mime::TEXT_HTML_UTF_8),
                Err(e) => {
                    tracing::error!(
                        msg = "fastn-Error",
                        path = path.as_str(),
                    ...
                    fastn::server_error!("fastn-Error: path: {}, {:?}", path, e)
                }
            }
        }
        fastn::File::Image(image) => {
            fastn::http::ok_with_content_type(image.content, guess_mime_type(image.id.as_str()))
        }
        fastn::File::Static(s) => fastn::http::ok(s.content),
        _ => {
            tracing::error!(msg = "unknown handler", path = path.as_str());
            fastn::server_error!("unknown handler")
        }
    }
}
...
async fn serve_cr_file(
        ...
        .await
    {
        Ok(f) => f,
        Err(e) => {
            return fastn::server_error!("fastn-Error: path: {}, {:?}", path, e);
        }
    };
    ...
    match f {
        ...
        fastn::File::Static(s) => fastn::http::ok(s.content),
        _ => {
            fastn::server_error!("fastn unknown handler")
        }
    }
}
...
#[tracing::instrument(skip_all)]
async fn serve_fastn_file(config: &fastn::Config) -> fastn::http::Response {
...
#[tracing::instrument(skip_all)]
async fn static_file(file_path: camino::Utf8PathBuf) -> fastn::http::Response {
    if !file_path.exists() {
        tracing::error!(msg = "no such static file ({})", path = file_path.as_str());
        return fastn::not_found!("no such static file ({})", file_path);
    }

    match tokio::fs::read(file_path.as_path()).await {
        Ok(r) => fastn::http::ok_with_content_type(r, guess_mime_type(file_path.as_str())),
        Err(e) => {
            tracing::error!(
            ...
            fastn::not_found!("fastn-Error: path: {:?}, error: {:?}", file_path, e)
        }
    }
}
...
pub async fn serve(
    ...
    let response = if path.eq(&favicon) {
        ...
        let mut config = fastn::Config::read(None, false, Some(&req))
            .await
            .unwrap()
            .add_edition(edition)?
            .set_request(req)
    ...
    } else if let Some(cr_number) = fastn::cr::get_cr_path_from_url(path.as_str()) {
        let mut config = fastn::Config::read(None, false, Some(&req))
            .await
            .unwrap()
            .add_edition(edition)?
        ...
        // fastn::file::is_static: checking for static file, if file is static no need to redirect it.
        if req_method.as_str() == "GET" && !fastn::file::is_static(path.as_str())? {
                ...
                .map(|x| (&x.mount_point, &x.package))
            {
                if let Some(remaining_path) =
                    fastn::config::utils::trim_package_name(path.as_str(), dep.name.as_str())
                {
        ...
        if file_response.status() == actix_web::http::StatusCode::NOT_FOUND {
            ...
            let (package_name, url, mut conf) =
                fastn::config::utils::get_clean_url(&config, path.as_str())?;
            ...
            let req = if let Some(r) = config.request {
                ...
                return Ok(fastn::server_error!("request not set"));
            };
        ...
        //         return Ok(fastn::server_error!("request not set"));
        //     };
        //     fastn::wasm::handle_wasm(req, wasm_module, config.package.backend_headers).await
        // }
...
pub async fn clear_cache(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    fn is_login(req: &fastn::http::Request) -> bool {
        // TODO: Need refactor not happy with this
        req.cookie(fastn::auth::AuthProviders::GitHub.as_str())
            .is_some()
            || req
                .cookie(fastn::auth::AuthProviders::TeleGram.as_str())
                .is_some()
            || req
                .cookie(fastn::auth::AuthProviders::Discord.as_str())
                .is_some()
            || req
                .cookie(fastn::auth::AuthProviders::Slack.as_str())
                .is_some()
            || req
                .cookie(fastn::auth::AuthProviders::Google.as_str())
                .is_some()
    }
    ...
    if from.from.eq(&Some("temp-github".to_string())) {
        ...
        return Ok(fastn::apis::cache::clear(&req).await);
    }
    ...
    let _lock = LOCK.write().await;
    fastn::apis::cache::clear(&req).await;
...
async fn sync(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::sync(&req, req.json()?).await
}

async fn sync2(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::sync2(&req, req.json()?).await
}

pub async fn clone(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.read().await;
    fastn::apis::clone(req).await
}

pub(crate) async fn view_source(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.read().await;
    Ok(fastn::apis::view_source(&req).await)
}

pub async fn edit(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::edit(&req, req.json()?).await
}

pub async fn revert(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::edit::revert(&req, req.json()?).await
}

pub async fn editor_sync(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::edit::sync(req).await
}

pub async fn create_cr(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.write().await;
    fastn::apis::cr::create_cr(&req, req.json()?).await
}

pub async fn create_cr_page(req: fastn::http::Request) -> fastn::Result<fastn::http::Response> {
    let _lock = LOCK.read().await;
    fastn::apis::cr::create_cr_page(req).await
}
...
async fn route(
    ...
    if req.path().starts_with("/auth/") {
        return fastn::auth::routes::handle_auth(
            req,
    ...
    let req = fastn::http::Request::from_actix(req, body);
    match (req.method().to_lowercase().as_str(), req.path()) {
        ...
        ("get", "/-/poll/") => fastn::watcher::poll().await,
        (_, _) => {
            serve(
                req,
                app_data.edition.clone(),
...
pub async fn listen(
    ...
    let tcp_listener = match fastn::http::get_available_port(port, bind_address) {
