...
fn expression_i() {
    ...
    let function = Function::new(|argument| {
        let arguments = argument.as_tuple()?;
        if let Value::Tuple(a) = &arguments[0] {
            let mut a = a.clone();
            a.push(arguments[1].clone());
            Ok(Value::Tuple(a.clone()))
        } else {
            Err(EvalexprError::expected_tuple(argument.clone()))
        }
    });

    let f = Function::new(|argument| {
    ...
    .unwrap();
    let g = evalexpr::eval_string_with_context_mut(
        "a = \"hello\" + \"world\"; y=4; b=2 + y; c = f((2, 3), 4); g = \"\"; a",
        &mut context,
    );
    dbg!(&g, &context);
}
