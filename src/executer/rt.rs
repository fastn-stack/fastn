...
impl RT {
    ...
    //         Some(ftd::interpreter::Thing::Variable(v)) => match v.value {
    ...
    //                 Ok(old)
    //             }
    //             ref t => ftd::interpreter::utils::e2(
    //                 format!("{} is not a boolean", variable),
    //                 format!("{:?}", t).as_str(),
    //             ),
    //         },
    //         Some(t) => ftd::interpreter::utils::e2(
    //             format!("{} is not a variable", variable),
    //             format!("{:?}", t).as_str(),
    //         ),
    //         None => ftd::interpreter::utils::e2(format!("{} not found", variable), doc_id),
    //     }
    // }
    ...
    pub fn render_(&mut self) -> ftd::p1::Result<ftd::Column> {
        let mut main = ftd::interpreter::interpreter::default_column();
        ...
        ftd::interpreter::document::set_region_id(&mut element);
        ftd::interpreter::document::default_scene_children_position(&mut element);
