...
impl ftd::html1::Action {
    ...
    pub(crate) fn from_function_call(
        ...
        ftd::html1::Action::new(function_call.name.as_str(), values)
    }
    ...
    fn to_list(self) -> Vec<ftd::html1::Action> {
        vec![self]
    }
}
...
pub(crate) fn group_by_js_event(
    ...
    for event in evts {
        if let Some(actions) = events.get_mut(to_event_name(&event.name)) {
            actions.push(ftd::html1::Action::from_function_call(&event.action));
        } else {
            events.insert(
                to_event_name(&event.name).to_string(),
                ftd::html1::Action::from_function_call(&event.action).to_list(),
            );
        }
    }
