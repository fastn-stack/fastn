...
pub fn optimise_wasm() -> fastn_core::Result<()> {
    ...
    let binaryen_version = env::var("BINARYEN_VERSION").unwrap_or_else(|_| DEFAULT_BINARYEN_VERSION.to_string());
...
fn ensure_wasm_opt(binaryen_version: &str) -> fastn_core::Result<String> {
    ...
    let binary_name = match os {
        ...
        _ => return Err(fastn_core::Error::GenericError(format!("Unsupported platform: {}", os))),
    };
    ...
    fs::create_dir_all("./bin")
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to create bin directory: {}", e)))?;
    ...
    if !Path::new(&local_install_dir).exists() {
        ...
        let extract_status = Command::new("tar")
            ...
            .status()
            .map_err(|e| fastn_core::Error::GenericError(format!("Failed to extract archive: {}", e)))?;
        
        if !extract_status.success() {
            return Err(fastn_core::Error::GenericError("Failed to extract binaryen archive".to_string()));
        }
        
        println!("Removing {}...", binary_name);
        fs::remove_file(&binary_name)
            .map_err(|e| fastn_core::Error::GenericError(format!("Failed to remove archive: {}", e)))?;
    }
    
    let wasm_opt_path = format!("{}/bin/wasm-opt", local_install_dir);
    if !Path::new(&wasm_opt_path).exists() {
        return Err(fastn_core::Error::GenericError("wasm-opt not found in the extracted files".to_string()));
    }
...
fn download_release(repo_name: &str, version: &str, binary_name: &str) -> fastn_core::Result<()> {
    let url = format!("https://github.com/{}/releases/download/{}/{}", repo_name, version, binary_name);
    println!("Downloading release from {}", url);
    
    let status = Command::new("curl")
        .args(["-L", "-o", binary_name, &url])
        .status()
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to download binaryen: {}", e)))?;
    
    if !status.success() {
        return Err(fastn_core::Error::GenericError(format!("Failed to download {} from {}", binary_name, url)));
    }
...
fn optimise_wasm_file(wasm_opt_cmd: &str, wasm_file: &Path) -> fastn_core::Result<()> {
    ...
    let before_size = fs::metadata(wasm_file)
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to get file metadata: {}", e)))?
        .len();
    
    let status = Command::new(wasm_opt_cmd)
        .args(["-Oz", &wasm_file.to_string_lossy(), "-o", &wasm_file.to_string_lossy()])
        .status()
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to run wasm-opt: {}", e)))?;
    
    if !status.success() {
        return Err(fastn_core::Error::GenericError("Optimization failed".to_string()));
    }
