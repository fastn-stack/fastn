...
fn update_gitignore() -> fastn_core::Result<()> {
    ...
    if fs::metadata(gitignore_path).is_ok() {
        fs::remove_file(gitignore_path)
            .map_err(|e| fastn_core::Error::GenericError(format!("Failed to remove existing .gitignore: {}", e)))?;
    }
    
    let mut file = fs::File::create(gitignore_path)
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to create .gitignore: {}", e)))?;
    
    writeln!(file, ".packages")
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to write to .gitignore: {}", e)))?;
    writeln!(file, ".fastn")
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to write to .gitignore: {}", e)))?;
    writeln!(file, ".is-local")
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to write to .gitignore: {}", e)))?;
...
fn install_fastn() -> fastn_core::Result<()> {
    ...
    if !status.success() {
        return Err(fastn_core::Error::GenericError("Failed to install fastn".to_string()));
    }
...
fn upload_to_fastn(site_dir: &PathBuf) -> fastn_core::Result<()> {
    env::set_current_dir(site_dir)
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to change to site directory: {}", e)))?;
    
    let status = Command::new("fastn")
        .args(["upload", "test"])
        .status()
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to run fastn upload: {}", e)))?;
    
    if !status.success() {
        return Err(fastn_core::Error::GenericError("fastn upload failed".to_string()));
    }
