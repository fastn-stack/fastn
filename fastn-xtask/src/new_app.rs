pub async fn new_app(app_name: &str) -> fastn_core::Result<()> {
    ...
    let mut archive = zip::ZipArchive::new(cursor).map_err(|e| {
        fastn_core::Error::GenericError(format!("Failed to read zip archive: {}", e))
    })?;

    let temp_dir = std::env::temp_dir().join(format!("fastn-template-{}", app_name));
    if temp_dir.exists() {
        std::fs::remove_dir_all(&temp_dir).map_err(|e| {
            fastn_core::Error::GenericError(format!("Failed to clean temp directory: {}", e))
        })?;
    }

    for i in 0..archive.len() {
        let mut file = archive.by_index(i).map_err(|e| {
            fastn_core::Error::GenericError(format!("Failed to read zip entry: {}", e))
        })?;
        ...
        if file.name().ends_with('/') {
            std::fs::create_dir_all(&outpath).map_err(|e| {
                fastn_core::Error::GenericError(format!("Failed to create directory: {}", e))
            })?;
        } else {
            ...
            let mut outfile = std::fs::File::create(&outpath).map_err(|e| {
                fastn_core::Error::GenericError(format!("Failed to create file: {}", e))
            })?;
            std::io::copy(&mut file, &mut outfile).map_err(|e| {
                fastn_core::Error::GenericError(format!("Failed to extract file: {}", e))
            })?;
        }
    }
    ...
    let extracted_dirs: Vec<_> = std::fs::read_dir(&temp_dir)
        .map_err(|e| {
            fastn_core::Error::GenericError(format!("Failed to read temp directory: {}", e))
        })?
        .filter_map(|entry| {
            let entry = entry.ok()?;
    ...
    if extracted_dirs.is_empty() {
        return Err(fastn_core::Error::GenericError(
            "No directories found in extracted zip".to_string(),
        ));
    }
    ...
    std::fs::rename(source_dir, target_path).map_err(|e| {
        fastn_core::Error::GenericError(format!("Failed to move extracted files: {}", e))
    })?;
            ...
            if script_path.exists() {
                if let Ok(contents) = std::fs::read_to_string(&script_path) {
                    ...
                    std::fs::write(&script_path, new_contents)
                        .map_err(|e| {
                            eprintln!("Failed to update {} in scripts: {}", script_file, e)
                        })
                        .ok();
                }
            }
