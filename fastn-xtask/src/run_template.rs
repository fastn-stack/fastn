...
pub fn run_template() -> fastn_core::Result<()> {
    ...
    let entries = std::fs::read_dir(&current_dir)
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to read current directory: {}", e)))?;
    ...
    env::set_current_dir(template_dir)
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to change to template directory: {}", e)))?;
    ...
    let status = Command::new(fastn_bin)
        ...
        .status()
        .map_err(|e| fastn_core::Error::GenericError(format!("Failed to execute fastn serve: {}", e)))?;
    
    if !status.success() {
        return Err(fastn_core::Error::GenericError("fastn serve failed".to_string()));
    }
...
fn get_fastn_binary() -> fastn_core::Result<String> {
    ...
    Err(fastn_core::Error::GenericError("Could not find fastn binary".to_string()))
}
