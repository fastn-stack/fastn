...
pub fn generate_project_id() -> String {
    let current_dir = std::env::current_dir().expect("Cannot read current directory");
    ...
    let fastn_content = std::fs::read_to_string(&fastn_ftd_path).unwrap_or_default();
        ...
        .output()
    {
        if output.status.success() {
            ...
            // Get repo name
            let repo_name = get_git_repo_name(&current_dir).unwrap_or_else(|| {
                git_root_path
                    .file_name()
                    .unwrap_or_default()
                    .to_string_lossy()
...
mod tests {
    ...
    fn test_filesystem_sanitization() {
        assert_eq!(
            sanitize_for_filesystem("path/with\\colon:"),
            "path_with_colon_"
        );
    }
}
